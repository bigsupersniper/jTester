...
(function() {
  var FormData, HTTP, Path, QueryString, URL, app, config, e, fs, nw, rootdir;
  ...
  app.run(function($templateCache) {
    ...
    return $templateCache.put(jTester.global.templateUrls.savefile, fs.readFileSync(jTester.global.templateUrls.savefile, {
      encoding: "utf-8"
    }));
  });
  ...
  window.jTester.http = (function() {
    ...
    http.prototype.down = function() {
      ...
      return HTTP.get(options, function(res) {
        ...
        return res.on('data', function(chunk) {
          ...
          file.end();
          return jTester.alert.success("" + file.path + " 已下载完成");
        }).on('error', function(e) {
    ...
    http.prototype.upload = function() {
      ...
      this.$context.$modalInstance.close('dismiss');
      url = URL.resolve(jTester.config.host, "/" + this.params.controller + "/" + this.params.action + "/");
      return form.submit(url, jTester.config.headers, function(err, res) {
        var dataType;
        if (err) {
          _this.action.submit = false;
